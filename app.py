...
fig1 = px.line(df_grouped, x="year", y="total_inactive_population", color="sex",color_discrete_map={'male':'blue', 'female':'red'}, title="Total Inactive Population By Sex")
...
fig3 = px.line(df_grouped, x="year", y="total_unemployed_population", color="sex", color_discrete_map={'male':'blue', 'female':'red'}, title="Total Unemployed Population")
...
fig5 = px.line(df_grouped, x="year", y="total_employed_population", color="sex", color_discrete_map={'male':'blue', 'female':'red'}, title="Total Employed Population")
...
fig7 = px.pie(df_grouped, values='Basic_unemployment', names='sex', color='sex',
             color_discrete_map={'male':'orange', 'female':'pink'}, title="Unemployment status by basic education",hole=0.5)
st.plotly_chart(fig7)
fig8=px.bar(df,x='age_group',y='Basic_unemployment',color='sex',color_discrete_map={'male':'orange', 'female':'pink'},title='sum of unemployment by basic ed')
st.plotly_chart(fig8)
fig9 = px.pie(df_grouped, values='Intermediate_unemployment', names='sex', color='sex',
             color_discrete_map={'male':'blue', 'female':'grey'}, title="Unemployment status by intermediate education",hole=0.5)
st.plotly_chart(fig9)
fig10=px.bar(df,x='age_group',y='Intermediate_unemployment',color='sex',color_discrete_map={'male':'blue', 'female':'grey'},title='sum of unemployment by intermediate ed')
